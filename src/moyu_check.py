...
from moyu_constance import detect_lingqu_renwu, detect_huoxianzhuijiao, \
    detect_my_location_x, detect_my_location_y, detect_fang_waiguai, \
    click_cancel_leftzidongxunlu, click_cancel_centerzidongxunlu, detect_bb_shishenm, \
    detect_siwang_point, click_huanshou_gezi1, detect_zidongxunlun_left1, detect_zidongxunlun_left2, \
    detect_zidongxunlun_center, detect_xp0, detect_xp00
...
def checkZiDongXunLu(ocr, scaleScreen):
    ...
    x, y = realPoint(detect_zidongxunlun_left1, scaleScreen)
    # 100,100 是像素点的坐标
    data1 = src_strlist[x, y]
    x, y = realPoint(detect_zidongxunlun_left2, scaleScreen)
    # 100,100 是像素点的坐标
    data2 = src_strlist[x, y]
    if data1 == (36, 18, 20) and data2 == (67, 20, 24):
...
# (232, 183, 241)
# 检测有没有XP
def checkHasXp(scaleScreen):
    img_src = Image.open('./screenshot.png')  # (411, 273)
    src_strlist = img_src.load()
    x, y = realPoint(detect_xp0, scaleScreen)
    # 100,100 是像素点的坐标
    data = src_strlist[x, y]
    if data == (255, 160, 49):
        return 1
...
# 检测XP是否结束
def checkHasXpIsFinish(scaleScreen):
    img_src = Image.open('./screenshot.png')  # (411, 273)
    src_strlist = img_src.load()
    x, y = realPoint(detect_xp00, scaleScreen)
    # 100,100 是像素点的坐标
    (r, g, b) = src_strlist[x, y]
    if r > 170:
        return 0
    return 1
